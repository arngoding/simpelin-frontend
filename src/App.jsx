...
export default function App() {
  ...
  const handleImportSubmit = (e) => {
    ...
    if (newItems.length > 0) {
      ...
      showToast('Format data tidak valid. Gunakan format: Nama, Kode Barang, StokAwal, Kategori, Satuan', 'error');
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gray-100 flex flex-col md:flex-row">
      ...
      {/* --- SIDEBAR --- */}
      <aside className="bg-slate-800 text-white w-full md:w-64 flex-shrink-0 md:min-h-screen relative">
          ...
          <div className="bg-white rounded-xl shadow-2xl max-w-md w-full p-6">
            ...
            <form onSubmit={handleSaveUser}>
              <div className="space-y-4 mb-6">
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">ID Pengguna (Login ID)</label>
                  <input 
                    type="text" 
                    className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none"
                    value={userForm.id}
